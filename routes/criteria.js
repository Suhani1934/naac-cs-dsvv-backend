...
// GET all criteria with total sub-heading count
...
router.post("/", verifyAdmin, async (req, res) => {
  try {
    ...
    if (!criterionNumber || !name)
      return res
        .status(400)
        .json({ error: "Criterion number and name required" });
...
/**
 * GET all main headings (CriterionDetails) for a criterion
 */
...
/**
 * POST - Add main heading (CriterionDetail) to a criterion
 */
...
router.put("/detail/:id", verifyAdmin, async (req, res) => {
  try {
    const { mainSerialNumber, mainTitle } = req.body;
    const updated = await CriterionDetail.findByIdAndUpdate(
      req.params.id,
      { mainSerialNumber, mainTitle },
      { new: true }
    );
    res.json(updated);
  } catch (err) {
 ...
 // DELETE - Delete main heading with all subheadings
...
router.post("/detail/:id/sub", verifyAdmin, async (req, res) => {
  try {
    ...
    // if any field is missing, return 400
    if (!serialNumber || !title || !link) {
      return res.status(400).json({ error: "All fields are required" });
    }

    const detail = await CriterionDetail.findById(req.params.id);
    if (!detail) return res.status(404).json({ error: "Main heading not found" });
...
router.put("/detail/:id/sub/:subId", verifyAdmin, async (req, res) => {
  try {
    ...
    if (!sub) return res.status(404).json({ error: "Subheading not found" });
